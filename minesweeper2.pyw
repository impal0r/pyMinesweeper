...
class MinesweeperGame(tk.Frame):
    ...
    def __init__(self, master, root, width, height, bomb_number=None, bomb_density=1/6, bombs=None, scale=SCALE):
        #SET UP LOGICAL GRID
        #initialise grid
        self.width = int(width)
        self.height = int(height)
        assert self.width > 0 and self.height > 0, 'Grid size must be positive'
        self.grid = [[0 for j in range(self.height)] for i in range(self.width)]
        ...
        if bombs is not None: #if list specified set bombs from list
            ...
            for bomb in bombs:
                ...
                if bomb[0]<0 or bomb[0]>=self.width or bomb[1]<0  or bomb[1]>=self.height:
                    ...
                    continue
                self.grid[bomb[0]][bomb[1]] = 16 #denotes bomb
        ...
        self.SQUARE = 16*scale                     #size of square in px
        ...
        for i in range(self.width):
            for j in range(self.height):
                if not self.grid[i][j]: #not a bomb -> still 0
                    #work out number in square
                    count = 0
                    for x,y in self.neighbours(i,j):
                        if self.grid[x][y] is 16:
                            count += 1
                    self.grid[i][j] = count
    ...
    def lose(self, x, y): #x and y are coordinates of bomb clicked
        ...
        for i in range(self.width):
            for j in range(self.height):
                ...
                if self.grid[i][j] & 0b100000 and not self.grid[i][j] & 0b10000:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.button == LEFT and (x,y) == self.last_leftbtndown_sq:
                    if self.grid[x][y] & 0b1000000: #if button removed
                        self.num_click(x, y)
                    else:
    ...
    def clear_space(self, x, y):
        ...
        while Q:
            ...
            for i,j in self.neighbours(x,y):
                if not self.grid[i][j] & 0b1000000: #if button not removed
                    if not self.grid[i][j] & 0b0100000:
                        self.hide_button(i, j) #only expose if not flagged
                    if not self.grid[i][j] & 0b11111: #only check number
                        Q.append((i,j))

    def button_click(self, x, y):
        if not (self.stop or self.grid[x][y] & 0b1100000): #if not flagged or removed
            self.hide_button(x, y)
            #if bomb, lose
            if self.grid[x][y] & 0b11111 > 15:
                if self.virgin: #if this is the first click, move bomb to random square
                    #make not bomb
            ...
            if not self.grid[x][y] & 0b11111: #only check number
    ...
    def hide_button(self, x, y):
        if not self.grid[x][y] & 0b1000000: #if not removed already
            self.SCREEN.blit(self.imgs[self.grid[x][y]&0b11111],(self.SQUARE*x,self.SQUARE*y))
            self.grid[x][y] |= 0b1000000 #set removed bit
            self.buttons_left -= 1

    def button_flag(self, x, y):
        if not self.stop:
            if self.grid[x][y] & 0b0100000: #if flagged
                self.SCREEN.blit(self.btn_img,(self.SQUARE*x,self.SQUARE*y))
                self.grid[x][y] &= 0b1011111 #clear flagged bit
                self.bomb_ctr += 1
                self.bombCounter.config(text=str(self.bomb_ctr))
            else:
                self.SCREEN.blit(self.flagged_img,(self.SQUARE*x,self.SQUARE*y))
                self.grid[x][y] |= 0b0100000 #set flagged bit
                self.bomb_ctr -= 1
                self.bombCounter.config(text=str(self.bomb_ctr))
    ...
    def num_click(self, x, y):
        ...
        flags = 0
        for i,j in self.neighbours(x,y):
            if self.grid[i][j] & 0b0100000: #flagged
                flags += 1

        if self.grid[x][y] & 0b1111 is flags: #if right number of cells flagged (=number)
            for i,j in ((x-1,y-1),(x,y-1),(x+1,y-1),
                        (x-1,y),(x+1,y),
                        (x-1,y+1),(x,y+1),(x+1,y+1)):
                if not(i<0 or j<0 or i==self.width or j==self.height):
                    if not self.grid[i][j] & 0b0100000: #not flagged
    ...
    def get_number(self, x, y):
        ...
        if x<0 or y<0 or x>=self.width or y>=self.height: #invalid coords
            return -1
        elif self.grid[x][y] & 0b10000: #if bomb
            return -1
        else:
            return self.grid[x][y] & 0b1111 #get just number
    ...
    def is_exposed(self, x, y):
        ...
        return self.grid[x][y] & 0b1000000 #button removed bit

    def is_flagged(self, x, y):
        '''Return whether this grid square is flagged'''
        assert x>=0 or y>=0 or x<self.width or y<self.height, 'invalid co-ordinates'
        return self.grid[x][y] & 0b0100000 #flagged bit
    ...
    def add_bomb(self, x, y):
        ...
        if self.grid[x][y] & 0b1010000: #if button removed or bomb already
            return False
        else:
            self.grid[x][y] |= 0b10000 #set bomb bit
            self.grid[x][y] &= 0b1110000 #set number to 0
            self.bombs.append((x,y))
            self.bomb_number += 1
            self.bomb_ctr += 1
            self.bombCounter.config(text=str(self.bomb_ctr))
            if self.buttons_left == self.bomb_number:
                self.win() #if the new bomb number means we win, win
            #update numbers of neighbours
            for i,j in self.neighbours(x,y):
                if not self.grid[i][j] & 0b10000: #if not bomb
                    self.grid[i][j] += 1 #increment number
                    if self.grid[i][j] & 0b1000000: #if number exposed
                        self.SCREEN.blit(self.imgs[self.grid[i][j]&0b1111],
    ...
    def remove_bomb(self, x, y):
        ...
        if not self.grid[x][y] & 0b10000: #if not a bomb return false
            return False
        else:
            self.grid[x][y] &= 0b1100000 #clear bomb bit and set number to 0
            ...
            for i,j in self.neighbours(x,y):
                if not self.grid[i][j] & 0b10000: #if not bomb
                    self.grid[i][j] -= 1 #decrement number
                    if self.grid[i][j] & 0b1000000: #if number exposed show on screen
                        self.SCREEN.blit(self.imgs[self.grid[i][j]&0b1111],
                                         (i*self.SQUARE,j*self.SQUARE))
                else: #if bomb
                    count += 1
            self.grid[x][y] += count
            if self.grid[x][y] & 0b1000000: #if exposed show on screen
                self.SCREEN.blit(self.imgs[self.grid[x][y]&0b1111],(x*self.SQUARE,y*self.SQUARE))
            return True
...
if __name__=='__main__':
    ...
    game = MinesweeperGame(root, root, 30, 27, bomb_density=1/8)
    game.pack()
    root.mainloop()
